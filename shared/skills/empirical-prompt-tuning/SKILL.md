---
description: agent 向けテキスト指示（skill / slash command / task プロンプト / CLAUDE.md 節 / コード生成プロンプト）を、バイアスを排した実行者に動かしてもらい、両面（実行者の自己申告 + 指示側メトリクス）で評価して反復改善する手法。改善が頭打ちになるまで回す。プロンプトや skill を新規作成・大幅改訂した直後、エージェントの挙動が期待通りにならない原因を指示側の曖昧さに求めたいとき、「このスキルをチューニングして」「プロンプトを実測で改善して」と依頼されたときに使う。
metadata:
    source: mizchi/chezmoi-dotfiles (dot_claude/skills/empirical-prompt-tuning) をベースにローカル改変
...
1. **対象の特定**: 引数・会話からチューニング対象のプロンプト（ファイルパス）を特定する。特定できない場合はユーザーに確認する
2. **作業ファイルの作成**: イテレーション記録用のファイルを 1 つ作る（例: scratchpad や `/tmp` 配下に `ept-<対象名>.md`）。評価シナリオ・要件チェックリスト・各イテレーションの提示フォーマット出力をここに**追記**していく。反復は長丁場になりやすく、会話コンテキストの要約で「事前に固定したはずのチェックリスト」や過去イテレーションの数値が失われると評価の一貫性が崩れるため。各イテレーションの冒頭でこのファイルを読み直す
3. **シナリオの承認**: ワークフロー 1 で作る評価シナリオと要件チェックリストは、**最初の dispatch の前にユーザーに提示して承認を得る**。シナリオがずれていると以降の全イテレーションが無駄になる
...
2. **バイアス排除読み**: 指示を「白紙」の実行者に読ませる。Agent tool（環境によっては Task tool と呼ばれる。以下 Agent tool）で **新規 subagent を dispatch** する。自己再読で済ませない（直前に書いた文章を客観視することは構造的に不可能）。並列で複数シナリオを同時実行する場合は単一メッセージ内で複数 Agent 呼び出しを並べる。dispatch 不能環境の扱いは「環境制約」節を参照。
...
4. **両面評価**: 戻ってきた結果から次を記録する。
   ...
   - **指示側の計測**（判定規則は本節で一元定義、他箇所は本節を参照する）:
     ...
     - usage メタが取得できない環境では、subagent にレポート内で自己申告させる（概数で可、ただし全イテレーションで取り方を統一する）
...
各イテレーションで次の形でユーザーに提示し、起動処理で作った作業ファイルにも同じ内容を追記する:
...
以下は外部エコシステムの skill への参照。**この環境にインストールされていない場合は呼び出さない**（Skill tool でこれらの名前を指定してエラーになったら、単に無視して本 skill のワークフローを続行する）:
